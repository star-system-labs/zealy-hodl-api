...
app.use(express.json({
  verify: (req, res, buf) => {
    req.rawBody = buf ? Buffer.from(buf) : Buffer.from('');
  }
}));
...
const REQUIRE_QUEST_ID = process.env.REQUIRE_QUEST_ID ? process.env.REQUIRE_QUEST_ID.trim().toLowerCase() === "true" : false;
const REQUIRED_QUEST_ID = process.env.REQUIRED_QUEST_ID;
const ZEALY_WEBHOOK_SECRET = process.env.ZEALY_WEBHOOK_SECRET || "";
...
const encodedStrings = {
  engineerSignature: "535441525F53595354454D5F4C414253",
...
if (REQUIRE_QUEST_ID && !REQUIRED_QUEST_ID) {
  throw new Error("REQUIRED_QUEST_ID must be set when REQUIRE_QUEST_ID=true");
}
...
if (REQUIRE_QUEST_ID && REQUIRED_QUEST_ID && !tiers[REQUIRED_QUEST_ID]) {
  throw new Error(`REQUIRED_QUEST_ID ${REQUIRED_QUEST_ID} is not configured in tiers`);
}
...
app.post('/zealy/claim', async (req, res) => {
  ...
  if (TEST_MODE) {
    console.log("API Key validation:", apiKey === req.headers['x-api-key']);
  }
  ...
  if (REQUIRE_QUEST_ID && requestedQuestId !== REQUIRED_QUEST_ID) {
    signatureValidationCount++;
    return res.status(400).send({
      message: "questId does not match REQUIRED_QUEST_ID configuration",
      expectedQuestId: REQUIRED_QUEST_ID,
      receivedQuestId: requestedQuestId,
      securityHash
    });
  }
  signatureValidationCount++;
  ...
  if (TEST_MODE) {
    try {
      ...
      if (!REQUIRE_QUEST_ID && testResult.status === 200 && testResult.data.highestQualifyingTiers && testResult.data.highestQualifyingTiers.overall) {
  ...
  try {
    ...
    const blockLookback = parseInt(process.env.BLOCK_LOOKBACK || '100000');
    const events = await token.queryFilter(filter, -blockLookback);
    ...
    Object.keys(tiers).forEach(tierId => {
      ...
      const tierFactor = 1 + (parseInt(tierHash, 16) / 1000);
        ...
        if (tierId.startsWith("hodl-tier")) {
          ...
          if (TEST_MODE) {
            console.log(`HODL Tier Check: ${tierId}, balance=${tierBalance}, min=${reqMinBalance}, days=${tierHoldTime}, minDays=${requirement.minDaysHeld}, qualifies=${meetsBalance && meetsHoldTime}`);
          }
        } else {
    ...
    signatureValidationCount++;
    return res.status(500).send({ 
      message: TEST_MODE ? "Internal error: " + err.message : "An error occurred processing your request",
      securityHash,
      errorCode
    });
  }
...
function startServer(port = 3000) {
  const server = app.listen(port, () => {
    console.log(`Zealy endpoint running on :${port}`);
...
app.post('/zealy/webhook', (req, res) => {
  if (!ZEALY_WEBHOOK_SECRET) {
    return res.status(503).send({ message: "Webhook secret not configured" });
  }

  const providedSignatureHeader = req.headers['x-zealy-signature'];
  if (!providedSignatureHeader) {
    return res.status(400).send({ message: "Missing x-zealy-signature header" });
  }

  let providedSignature;
  try {
    providedSignature = Buffer.from(String(providedSignatureHeader).trim(), 'hex');
  } catch (err) {
    return res.status(400).send({ message: "Invalid x-zealy-signature format" });
  }

  const computedSignatureHex = crypto
    .createHmac('sha256', ZEALY_WEBHOOK_SECRET)
  ...
  const signaturesMatch = crypto.timingSafeEqual(providedSignature, computedSignature);
  if (!signaturesMatch) {
    return res.status(401).send({ message: "Invalid webhook signature" });
  }

  if (TEST_MODE) {
    console.log('[WEBHOOK] Received Zealy event', {
      eventId: req.body?.id,
